    ...
    <div className="max-w-7xl mx-auto px-3.5 sm:px-6 lg:px-8 py-4 sm:py-8 space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        <div className="flex items-center gap-2.5 sm:gap-3">
          <Link
            ...
            className="p-2 sm:p-2.5 bg-white border border-slate-200 shadow-xs rounded-xl text-slate-500 hover:text-orange-600 hover:border-orange-200 transition"
          >
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <div>
            <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight flex items-center gap-1.5 sm:gap-2">
              <CreditCard className="w-5 h-5 sm:w-6 sm:h-6 text-orange-600 flex-shrink-0" />
              <span>Payments Ledger — M{schedule.month_no}</span>
            </h1>
            <p className="text-[11px] sm:text-xs text-slate-500 font-medium">
              {schedule.committee_name} • Kist: <span className="text-orange-600 font-bold font-mono">₹{parseFloat(schedule.installment_per_member).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4">
        <div className="bg-white border border-slate-200 shadow-xs p-3.5 sm:p-5 rounded-2xl">
          ...
          <span className="text-[10px] text-slate-400 mt-0.5 block font-medium truncate">
            {memberGroups.filter(g => g.isAllPaid).length} members fully paid
          </span>
        </div>
        ...
        <div className="bg-white border border-slate-200 shadow-xs p-3.5 sm:p-5 rounded-2xl">
          ...
          <span className="text-[10px] text-slate-400 mt-0.5 block font-medium truncate">
            {memberGroups.filter(g => !g.isAllPaid).length} members awaiting
          </span>
        </div>
        ...
        <div className="bg-white border border-slate-200 shadow-xs p-3.5 sm:p-5 rounded-2xl">
          ...
          <span className="text-[10px] text-slate-400 mt-0.5 block font-medium">Remaining</span>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-3.5 sm:p-8 space-y-4 shadow-sm">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-sm sm:text-base font-extrabold text-slate-900">Member Contributions</span>
        ...
        {/* Mobile App Cards View (MNC Fintech App Style) */}
        <div className="md:hidden space-y-3">
          {memberGroups.map((g) => {
            ...
            return (
              ...
              >
                {/* Top Section */}
                <div className="p-3.5 pb-3 border-b border-slate-100">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center font-bold text-xs flex-shrink-0 shadow-2xs ${
                        g.isAllPaid 
                          ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' 
                          : 'bg-orange-50 text-orange-700 border border-orange-200'
                      }`}>
                        {g.member_name ? g.member_name.substring(0, 2).toUpperCase() : 'M'}
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="font-extrabold text-slate-900 text-sm truncate">{g.member_name}</span>
                          {hasMultipleSeats ? (
                        ...
                        {g.member_phone && (
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <a 
                              href={`tel:${g.member_phone}`} 
                              className="text-[11px] text-slate-500 hover:text-orange-600 font-mono flex items-center gap-1"
                            >
                    ...
                    <div className="flex-shrink-0">
                      {g.isAllPaid ? (
                        ...
                        <span className="inline-flex items-center gap-1 text-[10px] font-extrabold px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-200">
                          PARTIAL ({g.paid_count}/{g.payments.length})
                        </span>
                      )}
                    </div>
                  </div>

                  {/* 3-Col Mini Financial Widget */}
                  <div className="grid grid-cols-3 gap-2 mt-3 pt-2.5 border-t border-slate-100 bg-slate-50/80 p-2 rounded-xl text-center font-mono">
                    <div>
                      <span className="text-[9px] uppercase font-bold text-slate-400 tracking-wider block font-sans">Installment</span>
                      <span className="text-xs font-bold text-slate-800 mt-0.5 block">
                        ₹{g.total_installment.toLocaleString('en-IN', { minimumFractionDigits: 0 })}
                      </span>
                    </div>
                    <div>
                        ...
                        isEditingThisPenalty ? (
                          <div className="flex items-center justify-center gap-1 mt-0.5">
                            <input
                              ...
                              value={penaltyAmount}
                              onChange={(e) => setPenaltyAmount(e.target.value)}
                              className="w-14 bg-white border border-orange-300 rounded px-1 py-0.5 text-xs text-orange-700 text-center font-mono focus:outline-none"
                            />
                            <button onClick={() => handleSavePenalty(singlePayment.id)} className="p-0.5 text-emerald-600">
                              <Check className="w-3.5 h-3.5" />
                            </button>
                          </div>
                        ) : (
                          <span 
                            onClick={() => !isCompleted && handleOpenPenalty(singlePayment)}
                            className="text-xs font-bold text-amber-600 mt-0.5 block cursor-pointer underline decoration-dotted"
                            title="Tap to edit penalty"
                          >
                            ₹{g.total_penalty.toLocaleString('en-IN', { minimumFractionDigits: 0 })}
                          </span>
                        )
                      ) : (
                        <span 
                          onClick={() => toggleExpand(g.member_id)}
                          className="text-xs font-bold text-amber-600 mt-0.5 block cursor-pointer underline decoration-dotted"
                          title="Tap to expand seat penalty"
                        >
                          ₹{g.total_penalty.toLocaleString('en-IN', { minimumFractionDigits: 0 })}
                        </span>
                      )}
                    ...
                    <div>
                      <span className="text-[9px] uppercase font-bold text-slate-400 tracking-wider block font-sans">
                        {g.isPartial ? 'Pending Due' : 'Total Due'}
                      </span>
                      <span className={`text-xs font-black mt-0.5 block ${g.isAllPaid ? 'text-emerald-700' : 'text-slate-900'}`}>
                        ₹{(g.isPartial ? g.pending_due : g.total_due).toLocaleString('en-IN', { minimumFractionDigits: 0 })}
                      </span>
                    </div>
                  </div>
                  ...
                  {g.isAllPaid && g.latestPaidDate && (
                    <div className="text-[10px] text-slate-400 mt-1.5 text-right font-medium">
                      Paid on: <span className="font-semibold text-slate-600">{g.latestPaidDate.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                    </div>
                  )}
                </div>

                {/* Multi-seat breakdown accordion if expanded */}
                {hasMultipleSeats && isExpanded && (
                  <div className="p-3 bg-orange-50/20 border-b border-slate-100 space-y-2">
                    <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center justify-between px-0.5">
                      ...
                      <span className="font-mono text-slate-400">{g.payments.map(p => `#${p.seat_no}`).join(', ')}</span>
                    </div>
                      ...
                      {g.payments.map((p) => {
                        ...
                        return (
                          <div key={p.id} className="bg-white p-2.5 rounded-xl border border-slate-200 flex items-center justify-between text-xs">
                            <div>
                              <span className="font-bold text-slate-800 font-mono mr-1.5">Seat #{p.seat_no}</span>
                              <span className="text-slate-500 font-mono text-[11px]">₹{parseFloat(p.amount_paid).toLocaleString('en-IN')}</span>
                              {parseFloat(p.penalty_amount || 0) > 0 && (
                                <span className="text-amber-600 text-[10px] ml-1.5 font-mono">+₹{parseFloat(p.penalty_amount)} pen.</span>
                              )}
                            </div>
                            <div className="flex items-center gap-2">
                              <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-md ${
                                pPaid ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-amber-50 text-amber-700 border border-amber-200'
                              }`}>
                                {pPaid ? 'PAID' : 'PENDING'}
                              </span>
                              <button
                                ...
                                className={`text-[10px] font-bold px-2 py-1 rounded-lg border transition ${
                                  pPaid ? 'bg-slate-100 text-slate-600 border-slate-200' : 'bg-orange-600 text-white border-orange-600'
                                }`}
                              >
                                {pPaid ? 'Pending' : 'Pay'}
                              </button>
                ...
                <div className="p-2.5 bg-white rounded-b-2xl flex items-center gap-2">
                  {/* Multi-seat toggle */}
                  {hasMultipleSeats && (
                    <button
                      onClick={() => toggleExpand(g.member_id)}
                      className="px-2.5 py-2 rounded-xl text-xs font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 border border-slate-200 flex items-center gap-1 active:scale-95 transition"
                      title="Toggle individual seat breakdown"
                    >
                      <span>Seats</span>
                      {isExpanded ? <ChevronUp className="w-3.5 h-3.5 text-orange-600" /> : <ChevronDown className="w-3.5 h-3.5" />}
                    </button>
                  ...
                  {waLink ? (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-1 py-2 px-3 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 font-bold text-xs flex items-center justify-center gap-1.5 active:scale-98 transition shadow-2xs"
                    >
                      <MessageCircle className="w-3.5 h-3.5 text-emerald-600" />
                      <span>WhatsApp (₹{g.pending_due.toLocaleString('en-IN', { minimumFractionDigits: 0 })})</span>
                    </a>
                  ) : null}
                  ...
                  <button
                    onClick={() => hasMultipleSeats ? handleToggleMember(g.member_id) : handleTogglePayment(singlePayment.id)}
                    disabled={isCompleted}
                    className={`flex-1 py-2 px-3 rounded-xl text-xs font-bold transition border shadow-xs flex items-center justify-center gap-1 active:scale-98 ${
                      g.isAllPaid
                        ? 'bg-slate-100 hover:bg-slate-200 text-slate-700 border-slate-300'
                        : 'bg-orange-600 hover:bg-orange-700 text-white border-orange-600 shadow-orange-600/20'
                    } disabled:opacity-40`}
                  >
                    {g.isAllPaid ? (
                      <>
                        ...
                        <span>{hasMultipleSeats ? 'Mark Pending' : 'Mark Pending'}</span>
                      </>
                    ) : (
                      <>
                        <Check className="w-3.5 h-3.5 stroke-[2.5]" />
                        <span>{g.isPartial ? 'Mark All Paid' : (hasMultipleSeats ? 'Mark All Paid' : 'Mark Paid')}</span>
                      </>
                    )}
                  </button>
                </div>
