...
<div class="space-y-8" id="committeePortalRoot">
    ...
    <div class="glass-card p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-6 shadow-2xl relative overflow-hidden">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 pt-4 border-t border-slate-800 z-10 relative">
            ...
            <div class="bg-slate-900/80 p-3.5 rounded-2xl border border-slate-800">
                ...
                <span class="text-lg font-black text-amber-400 font-mono">{{ $committee->monthly_rate ?? $committee->deduction_rate }}%</span>
            </div>
            <div class="bg-slate-900/80 p-3.5 rounded-2xl border border-slate-800">
                <span class="block text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Zero Round</span>
                <span class="text-lg font-black text-teal-400 font-mono">Month {{ $committee->zero_deduction_month ?? ($committee->total_members - $committee->special_month_index + 1) }}</span>
            </div>
            <div class="bg-slate-900/80 p-3.5 rounded-2xl border border-slate-800">
                <span class="block text-[10px] font-semibold text-slate-400 uppercase tracking-wider">My Profile / Seats</span>
                <span class="text-sm font-bold text-emerald-300 truncate block">{{ $member->name }}</span>
                @if($totalSeats > 1)
                    <span class="text-[10px] text-purple-300 font-extrabold uppercase block mt-0.5">
                        <i class="fa-solid fa-users-rectangle mr-1"></i>{{ $totalSeats }} Registered Seats
                    </span>
                @endif
            </div>
            <div class="bg-slate-900/80 p-3.5 rounded-2xl border border-slate-800">
                ...
                <span class="text-sm font-bold text-white block mt-0.5">{{ $committee->start_date ? $committee->start_date->format('d M Y') : 'N/A' }}</span>
            </div>
            <div class="bg-slate-900/80 p-3.5 rounded-2xl border border-slate-800">
                ...
                <span class="text-sm font-bold text-emerald-300 block mt-0.5">{{ $committee->end_date ? $committee->end_date->format('d M Y') : 'N/A' }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-4">
        ...
        <div id="auctionBoardContainer" class="space-y-3">
            ...
            <div class="glass-card rounded-2xl border {{ $isLocked ? 'border-emerald-500/40' : 'border-slate-800' }} overflow-hidden transition-all"
                ...
                <div class="flex items-center justify-between p-4 cursor-pointer select-none"
                    ...
                    <div class="flex items-center space-x-3">
                        <!-- Month Badge -->
                        <div class="w-10 h-10 rounded-xl {{ $isLocked ? 'bg-emerald-500/20 border-emerald-500/30 text-emerald-400' : 'bg-slate-800 border-slate-700 text-slate-300' }} border font-black font-mono text-sm flex items-center justify-center shrink-0">
                            {{ $schedule->month_no }}
                        </div>
                        ...
                        <div>
                            <div class="flex items-center space-x-2 flex-wrap gap-y-1">
                                ...
                                @elseif($isBeforeDraw)
                                    <span class="px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-300 border border-blue-500/20 text-[10px] font-bold uppercase tracking-wider flex items-center space-x-1">
                                        ...
                                        <span>Bids Open on {{ $drawDate->format('d/m/Y') }}</span>
                                    </span>
                                @elseif($isTodayDraw)
                                ...
                                @elseif($isAfterDraw)
                                    <span class="px-2 py-0.5 rounded-full bg-slate-800 text-slate-400 border border-slate-700 text-[10px] font-bold uppercase tracking-wider flex items-center space-x-1">
                                        ...
                                        <span>Bidding Closed ({{ $drawDate->format('d/m/Y') }})</span>
                                    </span>
                                @endif
                            ...
                            <div class="flex items-center space-x-4 mt-0.5 text-[11px] text-slate-400 flex-wrap">
                                <span>Draw Date: <strong class="text-white font-mono">{{ $drawDate ? $drawDate->format('d/m/Y') : 'N/A' }}</strong></span>
                                <span>Net Payout: <strong class="text-emerald-300 font-mono">₹{{ number_format($schedule->net_payout, 2) }}</strong></span>
                                <span>Installment: <strong class="text-white font-mono">₹{{ number_format($schedule->installment_per_member, 2) }}</strong></span>
                                <span>Bids: <strong class="text-amber-400 font-mono bid-count-{{ $schedule->id }}">{{ $roundBids->count() }}</strong></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center space-x-3 shrink-0">
                        ...
                        @else
                            ...
                            @if($myBids->has($schedule->id))
                                <button type="button"
                                    ...
                                    <i class="fa-solid fa-pen-to-square mr-1"></i> Edit Bid (₹{{ number_format($myBids->get($schedule->id)->bid_amount, 0) }})
                                </button>
                            @else
                                <button type="button"
                        ...
                        <i class="fa-solid fa-chevron-down text-slate-500 text-xs transition-transform" id="chevron-{{ $schedule->id }}"></i>
                    </div>
                </div>

                <!-- Expandable Bids Section -->
                <div class="hidden border-t border-slate-800/80 p-4 space-y-3" id="round-body-{{ $schedule->id }}">
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <div class="p-3 rounded-xl bg-slate-900/80 border border-slate-800 text-center">
                            ...
                            <span class="block font-mono font-bold text-rose-300 text-sm">₹{{ number_format($schedule->deduction_amount, 2) }}</span>
                        ...
                        <div class="p-3 rounded-xl bg-slate-900/80 border border-slate-800 text-center">
                            ...
                            <span class="block font-mono font-bold text-emerald-400 text-sm">₹{{ number_format($schedule->net_payout, 2) }}</span>
                        </div>
                        <div class="p-3 rounded-xl bg-slate-900/80 border border-slate-800 text-center">
                            ...
                            @if($paidMemberCount === $totalMemberSeatsCount)
                                <span class="block text-xs font-bold text-emerald-300"><i class="fa-solid fa-check mr-1"></i>Paid {{ $totalMemberSeatsCount > 1 ? "({$paidMemberCount}/{$totalMemberSeatsCount})" : '' }}</span>
                            @elseif($paidMemberCount > 0)
                                <span class="block text-xs font-bold text-amber-300"><i class="fa-solid fa-clock mr-1"></i>Partial ({{ $paidMemberCount }}/{{ $totalMemberSeatsCount }})</span>
                            @else
                                <span class="block text-xs font-bold text-rose-400"><i class="fa-solid fa-clock mr-1"></i>Pending {{ $totalMemberSeatsCount > 1 ? "(0/{$totalMemberSeatsCount})" : '' }}</span>
                            @endif
                        </div>
                    </div>
                            ...
                            @else
                                ...
                                <div class="flex items-center justify-between p-2.5 rounded-lg {{ $bid->member_id == $member->id ? 'bg-emerald-500/10 border border-emerald-500/30' : 'bg-slate-900/60 border border-slate-800' }}">
                                    <div class="flex items-center space-x-2.5">
                                        ...
                                        @endif
                                        <div>
                                            <span class="block text-xs font-bold {{ $bid->member_id == $member->id ? 'text-emerald-300' : 'text-white' }}">
                                            ...
                                            @if($bid->remarks)
                                                <span class="block text-[10px] text-slate-400 italic">"{{ $bid->remarks }}"</span>
                                            @endif
                                        </div>
                                    </div>
